package gzlazypack.common.component.hibernate;
...
public abstract class HibernateSimpleDao {
	...
	protected Pagination findByCriteria(Criteria crit, int pageNo, int pageSize) {
		return findByCriteria(crit, pageNo, pageSize, true, true);
	}
	 ...
	 * @return
	 */
	protected Pagination findByCriteria(Criteria crit, int pageNo, int pageSize, boolean checkPassLastPage, boolean isSelectTotalCount) {
		...
		Pagination p = new Pagination(pageNo, pageSize, totalCount);
		if (isSelectTotalCount && totalCount < 1) {
			p.setList(new ArrayList());
			return p;
		}
